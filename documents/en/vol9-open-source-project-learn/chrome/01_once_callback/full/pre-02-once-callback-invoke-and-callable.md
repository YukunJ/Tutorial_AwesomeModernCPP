...
The key takeaway is that no matter which of the above cases `f` falls into, the calling syntax is always `std::invoke(f, args...)`. In your template code, you do not need to know the exact type of `f`—`std::invoke` internally dispatches it to the correct calling syntax for you.
